...
def main(twitter,p,i,nt):
    ...
    while  textpred != str(predclass) or  ( len(res)<40 or len(res)>230):
        try:
            if count<20:
                res=text_model.make_sentence(tries=100);
                print('res before nn :' +res)
                res=nt.generate_text(res, 35)
                print('res after  nn :' +res)
            else:
        ...
        while count2<20 and len(res)<230 and not (res[-1]=='.' or res[-1]=='?' or res[-1]=='"'):
            ...
            prevlen23=len(res)
            print('res before '+res)
            res=nt.generate_text(res,5)
            print('res after '+res)
            words=res.split()
            #res=nt.generate_text(res,count2)
            #words=res.split()
            ind=res.rfind(words[-1],prevlen23-len(words[-1])-1,200)
            print(str(words[-1])+' '+str(ind))
            splitres=res[:ind]
